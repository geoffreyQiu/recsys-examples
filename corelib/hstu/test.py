...
if sm_major_version == 9 and sm_minor_version == 0:
    from hstu_attn_interface import hstu_attn_varlen_func, hstu_attn_qkvpacked_func
...
def test_fused_attn(
    ...
    if run_benchmark is not None:
        ...
        for i in range(iterations):
            ...
            L_q, L_k, num_contexts, seq_offsets_q, seq_offsets_k, num_targets, qkv, q, k, v, rab, attn_mask = input_datas[i % 2]
            if run_benchmark == 0:
    ...
    L_q, L_k, num_contexts, seq_offsets_q, seq_offsets_k, num_targets, qkv, q, k, v, rab, attn_mask = generate_input(
        batch_size=batch_size,
        heads=heads,
        heads_rab=heads_rab,
        max_seq_len_q=max_seq_len_q,
    ...
    if not has_drab:
        ...
        if qkv is None:
            ...
            (dqkv_hstu) = torch.autograd.grad(
                hstu_out, qkv, g.view(-1, heads, hidden_dim), retain_graph=True,
            )
        ...
        if qkv is None:
            ...
            (dqkv_hstu, drab_hstu) = torch.autograd.grad(
                hstu_out, (qkv, rab), g.view(-1, heads, hidden_dim), retain_graph=True,
            )
            dq_hstu = dqkv_hstu[:, 0, :, :]
            dk_hstu = dqkv_hstu[:, 1, :, :]
            dv_hstu = dqkv_hstu[:, 2, :, :]
...
def test_fused_attn_fp8(
    ...
    if run_benchmark is not None:
        ...
        for i in range(iterations):
            if i == profiler_step_start:
                fwd_event_start.record()
            L_q, L_k, _, seq_offsets_q, seq_offsets_k, _, qkv, q, k, v, rab, attn_mask = input_datas[i % 2]
            if run_benchmark == 0:
                fwd_out = hstu_attn_varlen_func(
                    q=q.to(dtype),
                    k=k.to(dtype),
...
if __name__ == "__main__":
    test_paged_kv_attn(
        batch_size=1,
        heads=1,
        max_seq_len_q=2, # q is new history
        max_seq_len_k=2, # k is user feature + previous history  # kv cache = user feature + previous history + new history
        max_target_len=3, # target is target length
        attn_dim=128,
        hidden_dim=128,
        page_size=32,
        alpha=1.0,
