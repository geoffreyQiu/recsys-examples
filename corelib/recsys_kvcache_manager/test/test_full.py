...
def create_testing_kvcache_manager():
    ...
    print(f"[DEBUG] KVCache Host Memory Usage: {\
        kvcache_config.num_layers * \
...
if __name__ == "__main__":
    ...
    max_sequence_lengths = [
        2500,   1024,   1280,   2377,   1854,   1365,   2729,   2049,
        689,    1417,   1174,   1987,   596,    520,    1538,   1189
    ] 

    g_keys = [
        torch.randn((3, max_sequence_lengths[i], 4, 128), dtype=torch.bfloat16).cuda() for i in range(len(max_sequence_lengths))
    ]
    g_values = [
        torch.randn((3, max_sequence_lengths[i], 4, 128), dtype=torch.bfloat16).cuda() for i in range(len(max_sequence_lengths))
    ]
    ...
    def run_phase_1():
        ...
        assert torch.allclose(
            lookup_res.cached_start_indices,
        ...
        assert torch.allclose(
            kvcache_metadata.total_history_lengths,
    ...
    def run_phase_2():
        ...
        assert torch.allclose(
            lookup_res.cached_lengths,
            torch.tensor(cachedlen, dtype=torch.int32)
        )
        assert torch.allclose(
    ...
    def run_phase_5():
        ...
        index_meta, lookup_res = kvcache_mgr.lookup_kvcache(
            user_ids, sequence_lengths)
        assert torch.allclose(lookup_res.host_cached_lengths, torch.zeros_like(lookup_res.host_cached_lengths))

        kvcache_mgr.evict(user_ids, for_gpu=True, for_host=True)
        index_meta, lookup_res = kvcache_mgr.lookup_kvcache(
            user_ids, sequence_lengths)
        assert torch.allclose(lookup_res.gpu_cached_lengths, torch.zeros_like(lookup_res.host_cached_lengths))
