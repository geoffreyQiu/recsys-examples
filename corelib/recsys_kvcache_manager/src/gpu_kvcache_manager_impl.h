...
public:
    ...
    std::vector<int32_t>& alloc_single_sequence(int64_t uid, int new_total_length, int host_cached_startpos, int host_cached_length, std::unordered_set<int64_t> freezed_uids);
    void allocate(
    ...
    std::vector<at::Tensor, at::Tensor> lookup(at::Tensor uids);
    
    void evict(int64_t uid);
    void evict_all();

    std::vector<at::Tensor> check_for_offload(at::Tensor user_ids);
    std::vector<at::Tensor, at::Tensor, at::Tensor, at::Tensor> get_offload_pages(at::Tensor user_ids);
    void release_offload_pages(at::Tensor user_ids);
...
public:
    ...
    std::vector<void*> metadata_host_buffers;   // preallocated pinned host buffer
