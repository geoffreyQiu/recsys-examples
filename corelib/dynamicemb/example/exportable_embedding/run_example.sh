...
if [[ "${nve_version}" == "26.05" ]]; then
  ...
  dynamicemb_python_path="/opt/dynamicemb/26.05/python:${dynamicemb_root}"
else
  nve_root="/opt/nve/default/python"
  dynamicemb_python_path="${dynamicemb_root}"
fi

export DYNAMICEMB_OPS_LIB_DIR="${dynamicemb_root}/torch_binding_build"
export PYTHONPATH="${nve_root}:${dynamicemb_python_path}:${PYTHONPATH:-}"
