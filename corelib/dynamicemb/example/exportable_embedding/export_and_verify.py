...
def expected_after_update(
    ...
    for name in collection_names:
        ...
        for table_id in range(2):
            ...
            rows.append(changed[name][(table_id, new_feature_id)])
...
def run_four_round_update_workflow(
    *,
    ...
    def run_inference() -> None:
        with torch.cuda.stream(inference_stream), torch.inference_mode():
            ...
            for _ in range(REQUESTS_PER_VERIFIED_ROUND):
                ...
                assert_outputs(eager_outputs, expected_zero)
                assert_outputs(aoti_outputs, expected_zero)
            ...
            log_development_timing(
                ...
                "round_1_original",
                round_started,
                f"requests={REQUESTS_PER_VERIFIED_ROUND} sync_ms={sync_ms:.3f}",
            )
            round_one_done.set()
            ...
            for _ in range(REQUESTS_PER_VERIFIED_ROUND):
                ...
                assert_outputs(eager_outputs, state["expected_one"])
                assert_outputs(aoti_outputs, state["expected_one"])
            ...
            for _ in range(REQUESTS_PER_VERIFIED_ROUND):
                ...
                assert_outputs(eager_outputs, state["expected_two"])
                assert_outputs(aoti_outputs, state["expected_two"])
            sync_started = time.perf_counter()
            torch.cuda.synchronize(device)
            sync_ms = (time.perf_counter() - sync_started) * 1_000
            log_development_timing(
    ...
    def run_update() -> None:
        ...
        with torch.cuda.device(device):
            ...
            if cpp_process is not None:
                ...
                for acknowledgement in run_incremental_cpp_round(
            ...
            second_changed = {}
            second_updates = []
            # Apply bit-concat first and linear-hash last so the replaceable
            # snapshot is staged immediately before the retirement wait.
            for name in (BIT_CONCAT_REDIS, LINEAR_HASH_REDIS):
                delta, second_changed[name] = produce_delta(
                    name, producers[name], round_id=2
                )
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        update_task = executor.submit(run_update)
...
def stop_incremental_cpp_replayer(process: subprocess.Popen[str]) -> None:
    if process.stdin is None or process.stdout is None:
        raise RuntimeError("C++ replayer pipes are unavailable")
    ...
    try:
        ...
        if not enable_redis:
            with torch.inference_mode():
                verify_state(model, inputs, expected_zero)
                verify_state(aoti_runtime, inputs, expected_zero)
        ...
        try:
            run_four_round_update_workflow(
                model=model,
            ...
            if cpp_process is not None:
                stop_incremental_cpp_replayer(cpp_process)
                cpp_process = None
        finally:
            try:
