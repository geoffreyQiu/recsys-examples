...
// Reproduce the cache-invalidation window, then replace each per-tier erase
// with the corresponding per-tier update. Run as: <program> erase|update.
...
int main(int argc, char** argv) {
  ...
  CHECK_NVE(nve_host_factory_produce(
      redis_factory, 2,
      R"({"mask_size":8,"key_size":8,"max_value_size":4,"value_dtype":"float32","num_partitions":0,"string_namespace_id":991})",
      &redis_table));
  ...
  layer_config.layer_name = "cache_race_repro";
  ...
  insert(layer, layer_context, {7, 8}, {1.0f, 8.0f}, kRedisTier);
  insert(layer, layer_context, updated_key, old_value, kHostTier);
  insert(layer, layer_context, updated_key, old_value, kGpuTier);

  // Step 1: The original embedding is resident in both cache tiers.
  const float initial = lookup(layer, layer_context, updated_key)[0];
  std::printf("initial=%.1f\n", initial);

  // Step 2: The Redis value changes to 2.0.
  update(layer, layer_context, updated_key, new_value, kRedisTier);

  // Step 3: Change only tier 1. The update mode keeps the row resident.
  change_tier(operation, layer, layer_context, updated_key, new_value,
              kHostTier);

  // Step 4: A non-stop inference lookup runs between the two tier changes.
  const float between_tiers = lookup(layer, layer_context, {7, 8})[0];
  std::printf("between_tiers=%.1f\n", between_tiers);

  // Step 5: Change only tier 0.
  change_tier(operation, layer, layer_context, updated_key, new_value,
              kGpuTier);

  // Step 6: Inspect tier 1 before another hierarchical lookup can refill it.
  ...
  // Step 7: Compare the hierarchical result with the value stored in Redis.
  ...
  const bool verified =
      operation == Operation::kErase
          ...
          : (tier_1_after_changes == expected_from_redis &&
             after_tier_0_change == expected_from_redis);
  if (operation == Operation::kErase) {
    std::printf("RESULT original_mismatch_reproduced=%s\n",
                verified ? "true" : "false");
  } else {
