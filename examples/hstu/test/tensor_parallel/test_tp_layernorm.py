...
from commons.utils.distributed_utils import collective_assert_tensor
...
def test_tp_layernorm(
    ...
    tp_weight = ref_weight.detach().clone().requires_grad_(True) if trainable else None
    tp_bias = ref_bias.detach().clone().requires_grad_(True) if trainable else None
    hidden_start_this_rank = tp_rank * hidden_dim_per_partition
    hidden_end_this_rank = (tp_rank + 1) * hidden_dim_per_partition
    batchsize = 128
    eps = 1e-5
    ...
    for i in range(10):
        ...
        tp_x = (
            ref_x[
                ...,
                hidden_start_this_rank:hidden_end_this_rank,
            ]
            .detach()
            .clone()
            .requires_grad_(True)
...
def test_tp_layernorm_dropout_mul(hidden_dim, tp_size, sequence_parallel):
    ...
    with init.auto_destroy_global_state():
        ...
        # no need to broadcast the weight and bias because they are initialized the same on all ranks
        ref_weight = (
            torch.nn.Parameter(torch.ones(hidden_dim, device=device))
            if trainable
            else None
        )
        ref_bias = (
        ...
        for i in range(10):
            ...
            ref_u = (
                torch.empty(batchsize, hidden_dim, device=device, dtype=torch.bfloat16)
                .uniform_(-0.1, 0.1)
                .fill_(1.0)
            )
            # to ensure the same input within tp group
            ...
            if trainable:
                ref_tp_weight_grad = ref_weight.grad
                ref_tp_bias_grad = ref_bias.grad
                # SP resembles DP, so we need to all reduce the grad
                if sequence_parallel:
                    wgrad_to_compare = tp_layernorm_dropout_mul.weight.grad.clone()
                    bgrad_to_compare = tp_layernorm_dropout_mul.bias.grad.clone()
                    ...
                    wgrad_to_compare = tp_layernorm_dropout_mul.weight.grad
                    bgrad_to_compare = tp_layernorm_dropout_mul.bias.grad
                torch.testing.assert_close(wgrad_to_compare, ref_tp_weight_grad)
                torch.testing.assert_close(bgrad_to_compare, ref_tp_bias_grad)
