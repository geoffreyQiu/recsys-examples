...
def test_gr_infer(
    ...
    with torch.inference_mode():
        ...
        model_predict.prepare_kv_cache(batch, user_ids)
        ...
        input_values = torch.randn_like(jagged_data.values)
        jagged_data.values.copy_(input_values)

        num_tokens = batch.features.values().shape[0]
        model_predict._hidden_states[:num_tokens, ...].copy_(jagged_data.values, non_blocking=True)
        copy_jagged_metadata(model_predict._jagged_metadata, jagged_data)
        # print(">", model_predict._jagged_metadata.seqlen_offsets - model_predict._jagged_metadata.num_candidates_offsets)
        # print(">", model_predict._kvcache_metadata.total_history_offsets)
        model_predict._kvcache_metadata.total_history_offsets += model_predict._jagged_metadata.num_candidates_offsets
        # print(">>", model_predict._jagged_metadata.num_candidates_offsets)
        # print(">", model_predict._kvcache_metadata.total_history_offsets)
        hstu_output = model_predict._hstu_block.predict(batch.batch_size, num_tokens, model_predict._hidden_states, model_predict._jagged_metadata, model_predict._kvcache_metadata)
        model_predict.finalize_kv_cache(user_ids)
        torch.cuda.synchronize()


        print(hstu_output.values.shape)

        ref_output = get_reference_output(model_predict._hstu_block._attention_layers, batch.batch_size, input_values, model_predict._jagged_metadata, model_predict._kvcache_metadata)
        torch.cuda.synchronize()


        print(ref_output.shape)
        print(torch.allclose(hstu_output.values, ref_output))

        print(jagged_data.values.mean().item(), jagged_data.values.max().item())
        print(hstu_output.values.mean().item(), hstu_output.values.max().item())
        print(ref_output.mean().item(), ref_output.max().item())
        print()
        print(hstu_output.values.abs().mean().item(), hstu_output.values.abs().max().item())
        print(ref_output.abs().mean().item(), ref_output.abs().max().item())

        # print("===")
        # for j in range(0, ref_output.shape[0], 32):
        #     num_eq = 0
        #     idlist = list()
        #     for jj in range(j, j+32):
        #         num_eq += 1 if torch.allclose(hstu_output.values[jj, :], ref_output[jj, :]) else 0
        #         if not torch.allclose(hstu_output.values[jj, :], ref_output[jj, :]):
        #             idlist.append(jj)
        #     # print(j, j+32, num_eq / 32.0) 
        #     # print(j, j+32, idlist) 
        #     for jj in idlist:
        #         print(jj, torch.nonzero(hstu_output.values[jj, :]-ref_output[jj, :]).view(-1)[-1].item())
        #         print(hstu_output.values[jj, :16])
        #         print(ref_output[jj, :16])
        # print("===")

        print(((hstu_output.values-ref_output)/(ref_output + 1e-8)).abs().mean().item(), ((hstu_output.values-ref_output)/(ref_output + 1e-8)).abs().max().item())
        print(((hstu_output.values-ref_output)).abs().mean().item(), ((hstu_output.values-ref_output)).abs().max().item())
