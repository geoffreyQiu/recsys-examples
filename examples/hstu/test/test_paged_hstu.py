...
def generate_kvdata_testcase(
    ...
    values = torch.randn((2, num_tokens, num_heads, head_dim), dtype=torch.bfloat16, device=torch.cuda.current_device())
...
def setup_kvcache_testcase(
    ...
    if kvcache_table is None:
        kvcache_table = torch.zeros((2048, 2, page_size, kvcache_num_heads, kvcache_head_dim),
                                    dtype=kvcache_dtype, device=torch.cuda.current_device())
...
def test_gr_infer(
    ...
    with torch.inference_mode():
        ...
        user_ids = torch.arange(batchsize)
        kv_data, kvdata_seqlen = generate_kvdata_testcase(max_kvcache_history_seqlen, batchsize, num_heads, head_dim)
        kvcache_table, kv_raw_metadata = setup_kvcache_testcase(
            kvcache_page_size, num_heads, head_dim, dtype,
            kv_data, kvdata_seqlen, None)
        

        if max_num_candidates > 0:
            num_candidates = torch.randint(1, max_num_candidates+1, (batchsize,), dtype=torch.int32)
        else:
            num_candidates = torch.zeros((batchsize,), dtype=torch.int32)
        num_candidates_offsets = get_offsets_from_lengths(num_candidates)

        seqlen = torch.randint(max_new_history_seqlen//2, max_new_history_seqlen + 1, (batchsize,), dtype=torch.int32)
        ...
        value = torch.randn((num_tokens,num_heads,head_dim), dtype=dtype, device=device)
        ...
        mask = torch.zeros((batchsize, num_heads, max_seq_len, max_kvdata_len + max_target_len),
                           dtype=dtype, device=device)
        for seq_idx in range(batchsize):
            qlen = seqlen[seq_idx].item()
            cachelen = (kv_raw_metadata[1][seq_idx+1] - kv_raw_metadata[1][seq_idx] - 1) * kvcache_page_size + kv_raw_metadata[2][seq_idx]
