...
class AsyncHSTUKVCacheManager:
    def __init__(
        ...
        self.cache_table = torch.empty(
            [num_layers, self.num_cache_pages, 2, self.page_size, self.num_heads, self.head_dim],
        ...
        self.static_metadata_gpu_buffer = torch.empty([self.max_batch_size * 5 + 4 + self.max_batch_size * self.max_sequence_length * 2,], dtype=torch.int32).cuda()
    ...
    def prepare_kvcache_async(self, 
        ...
        return [origin_cached_lengths, new_tokens, offload_uids_buffer, metadata_host_buffer, static_metadata_gpu_buffer, kvcache_metadata_fut, onload_fut]
    ...
    def get_kvcache_metadata_from_buffer(self, 
        ...
        if int(metadata_host_buffer[batch_size * 7 + 5]) > 0:
            offload_handle = paged_kvcache_ops.KVOffloadHandle(self.num_layers, self.gpu_kvcache_mgr, True)
